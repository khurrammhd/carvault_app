...
- **`docs/privacy-policy.html` + `docs/play-store-data-safety.md`**: privacy policy text and a Data Safety questionnaire reference, both required by Play Console before submission. The privacy policy needs GitHub Pages enabled (Settings → Pages → Deploy from branch → `main` / `/docs`, manual) to get a public URL; see the data-safety doc for the exact URL and the Data Safety form answers.
...
- [ ] **Play Store submission — remaining manual steps** (release signing + CI are ready, see §6; none of these can be done from code):
  1. Add the four `ANDROID_*` secrets to GitHub repo Settings → Secrets and variables → Actions (values were generated locally when the upload keystore was created — not stored anywhere in the repo).
  2. Add the upload key's SHA-1 to Firebase now (§6); add the second, Play-App-Signing-generated SHA-1 after step 5.
  3. Enable GitHub Pages (Settings → Pages → `main` / `/docs`) so the privacy policy has a public URL.
