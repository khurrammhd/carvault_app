...
>
> **Last updated:** 2026-07-10 (release signing + Play Store prep)
...
- **Release signing uses Google Play App Signing, not a fully self-managed keystore.** The upload key (`android/keystores/upload-keystore.jks`) only authenticates uploads to Play Console — Google generates and holds the actual key used to sign what installs on user devices. Deliberate: with a self-managed key, losing the keystore file or its password means never being able to publish an update to that app listing again; with Play App Signing, Google can help recover from a lost upload key. The one-time cost is registering *two* SHA-1 fingerprints in Firebase instead of one (§6) — accepted given the alternative's failure mode is unrecoverable.
...
- **Release signing (Play Store):** uses **Google Play App Signing** — Google holds the real release signing key; the app only signs uploads with a separate "upload key" (`android/keystores/upload-keystore.jks`, alias `upload`), generated locally and **never committed** (`android/.gitignore` already excludes `key.properties` and `*.jks`, confirmed via `git check-ignore` before first commit). `android/app/build.gradle.kts` reads `android/key.properties` if present and falls back to the debug signing config if it's absent, so a checkout without it doesn't break local `flutter run --release`. Upload key SHA-1: `87:4E:08:7E:41:36:02:F7:D5:70:E4:7A:74:CB:5B:BB:DB:02:E4:E3` — **must be added to Firebase** (Authentication → Project settings → Add fingerprint) for Google Sign-In to work on anything signed with it; a **second** SHA-1, generated by Play App Signing itself, only becomes available after the first Play Console upload (App integrity → App signing key certificate) and must be added too, for Play-Store-installed builds specifically.
- **`.github/workflows/build-release-aab.yml`** (manual `workflow_dispatch` only, unlike the debug workflow's push-to-main trigger): rebuilds the upload keystore + `key.properties` from GitHub Actions secrets (`ANDROID_KEYSTORE_BASE64`, `ANDROID_KEYSTORE_PASSWORD`, `ANDROID_KEY_ALIAS`, `ANDROID_KEY_PASSWORD` — must be added under repo Settings → Secrets and variables → Actions, a manual step), runs `flutter build appbundle --release`, uploads `app-release.aab`, then deletes the reconstructed secrets from the runner regardless of outcome.
- **`docs/privacy-policy.html` + `docs/play-store-data-safety.md`**: privacy policy text and a Data Safety questionnaire reference, both required by Play Console before submission. **Live** at `https://khurrammhd.github.io/carvault_app/privacy-policy.html` (GitHub Pages enabled, Settings → Pages → `main` / `/docs`) — confirmed loading correctly.
- **Samsung Galaxy Store — deferred/blocked** (`docs/samsung-galaxy-store.md`): was the first choice for a second free distribution target (pre-installed on all Samsung devices), but Samsung requires **Corporate Commercial Seller status for *any* Android content, free or paid** — verified via a real error the user hit ("To sell Android content, a status change to corporate Commercial Distribution Seller is necessary"). That status needs a D‑U‑N‑S number (or DBA/equivalent) plus bank verification, i.e. a registered business — not available for an individual/hobby project, so this path is on hold unless that changes. The doc is kept for reference in case a business entity exists later.
- **Amazon Appstore** (`docs/amazon-appstore.md`): the actual second distribution target — a free **"Sole Proprietorship Developer Account"** works for individuals, no DUNS/business registration needed (identity verification only), unlike Samsung. Reuses the **same** `app-release.aab` from `build-release-aab.yml` — no separate CI workflow needed. One real difference from Samsung/Play: **Amazon always re-signs the app with its own account-level signing key** (can't keep your own), so a **third** SHA-1 fingerprint — shown in Amazon Developer Console after the app is first created — must be added to Firebase. It's per-account, not per-app, so this is a one-time registration covering any future apps under the same Amazon account.
...
- [ ] **Confirm CI is green on commit `273635e`** (the `workmanager` version fix, §8.7) — the prior push failed on a Kotlin compile error in the `workmanager` plugin; this commit should fix it, but the actual CI result hasn't been confirmed yet (tooling on this end hit a GitHub API rate limit while checking, and a raw page fetch of the Actions UI is unreliable — its status is loaded client-side by JavaScript, so it isn't in the fetched HTML). Check `https://github.com/khurrammhd/carvault_app/actions` directly.
...
- [ ] **Play Store submission — remaining manual steps** (release signing + CI are ready, see §6; none of these can be done from code):
  1. ✅ Add the four `ANDROID_*` secrets to GitHub repo Settings → Secrets and variables → Actions — done.
  2. Add the upload key's SHA-1 to Firebase (§6) — **not yet confirmed done**; add the second, Play-App-Signing-generated SHA-1 after step 5.
  3. ✅ Enable GitHub Pages (Settings → Pages → `main` / `/docs`) — done, privacy policy confirmed live.
  4. Create a Google Play Developer account (one-time $25 fee) and publish the OAuth consent screen for `carvault-e0e9a` in Google Cloud Console (needs the same privacy policy URL) — see `docs/play-store-data-safety.md` for both.
  5. Run the "Build Release AAB" GitHub Actions workflow manually, download `app-release.aab`, and upload it as the app's first release in Play Console.
  6. Fill in the Play Console Store Listing + Data Safety form (`docs/play-store-data-safety.md` has the answers) and submit for review.
- [ ] ~~Samsung Galaxy Store submission~~ — **on hold**, blocked by Samsung's Corporate Commercial Seller requirement (no individual/free-hobbyist path exists — see §6). Revisit only if a registered business entity becomes available; `docs/samsung-galaxy-store.md` has the steps ready for that case.
- [ ] **Amazon Appstore submission** (`docs/amazon-appstore.md`, replaces Samsung as the second distribution target) — free via a Sole Proprietorship account, reuses the same `app-release.aab`, no new CI needed:
  1. Create a free Amazon Developer account (Sole Proprietorship type — no DUNS/business needed) and register the app.
  2. Upload `app-release.aab` (same CI workflow as Play Store) — Amazon re-signs it automatically with an account-level key.
  3. Copy the Amazon account signature hash shown after app creation and add it to Firebase as a new SHA-1 fingerprint (one-time, covers future apps on the same account too).
  4. Fill in the listing using the same privacy policy URL and the data-disclosure notes in `docs/play-store-data-safety.md`; submit for review.
...
2. Confirm CI is green on `273635e` (§7), then work through the on-device verification list in §7 — this code has passed `flutter analyze` and the existing test suite locally, but the Drive auth flows, the WorkManager background isolate, and restore have **not yet been exercised on a real device**.
