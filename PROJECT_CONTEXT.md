...
- **Samsung Galaxy Store** (`docs/samsung-galaxy-store.md`): a second, free distribution target chosen over Amazon Appstore (Galaxy Store is pre-installed on all Samsung devices, so zero extra install friction; Amazon's Appstore isn't pre-installed on non-Amazon Android phones). Reuses the **same** `app-release.aab` from `build-release-aab.yml` — no separate CI workflow needed. Target SDK 36 and the arm64-v8a binary Flutter includes by default already satisfy Galaxy Store's requirements (≥33, ≥1 64-bit binary). Recommended signing approach: upload the *same* `upload-keystore.jks` into Samsung's Seller Portal rather than letting them generate a new one — keeps the already-registered Firebase SHA-1 valid for Galaxy-Store-installed builds too, avoiding a third fingerprint to manage.
...
- [ ] **Samsung Galaxy Store submission** (`docs/samsung-galaxy-store.md`, chosen over Amazon Appstore for pre-installed reach on Samsung devices) — free, reuses the same `app-release.aab`, no new keystore/CI needed:
  1. Create a free Samsung Developer account at seller.samsungapps.com and register the app in Seller Portal.
  2. Upload `app-release.aab` (from the same CI workflow as Play Store) and, when prompted for signing, upload the existing `upload-keystore.jks` rather than letting Samsung generate a new key — keeps the already-registered Firebase SHA-1 valid here too.
  3. Fill in the listing using the same privacy policy URL and the data-disclosure notes in `docs/play-store-data-safety.md`; submit for review.
